...
it up using the default source, which is the [Package site]({{site.pub}}.
...
A *hosted* package is one that can be downloaded from the Package site
...
[Package site][pubsite] if it has any path dependencies in its pubspec.
