...
Typically, APIs that return `Future` objects
fall into one of the following categories:

* APIs that are backed by logic running on a different thread. 
  For example, `File.readAsString` (and most of the other I/O APIs) 
  call operating system functions from threads on a dedicated thread pool, 
  rather than from the thread on which Dart code is executed.

* APIs that wait for an event of some kind and perform no work in the meantime.
  For example, `Future.delayed` waits for time to have elapsed, 
  and `Stream.first` waits for an event to be posted on the stream.
