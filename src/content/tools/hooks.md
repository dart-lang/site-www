...
Support for link hooks and recorded usage tree-shaking is available from
Dart 3.13.
:::
...
With link hooks, a package can filter, optimize, or tree-shake code assets
generated by build hooks before they are bundled into an application.

A package's link hook (`hook/link.dart`) is automatically invoked by the
Dart SDK during the application bundling phase. Unlike build hooks, which run
per package, link hooks run with application-level context so they can inspect
which symbols the application actually uses.

Use the [`link`][] function to parse the hook input with [`LinkInput`][] and
then write the hook output with [`LinkOutputBuilder`][].
If tree-shaking removes all used symbols for an asset, the link hook can omit
the asset entirely so it isn't bundled with the application.
...
[`CLibrary`]: {{site.pub-api}}/native_toolchain_c/latest/native_toolchain_c/CLibrary-class.html
[`CLibrary.build`]: {{site.pub-api}}/native_toolchain_c/latest/native_toolchain_c/CLibrary/build.html
[`CLibrary.link`]: {{site.pub-api}}/native_toolchain_c/latest/native_toolchain_c/CLibrary/link.html
[`link`]: {{site.pub-api}}/hooks/latest/hooks/link.html
[`LinkInput`]: {{site.pub-api}}/hooks/latest/hooks/LinkInput-class.html
[`LinkInput.recordedUses`]: {{site.pub-api}}/hooks/latest/hooks/LinkInput/recordedUses.html
...
### Create a link hook to tree-shake native assets {: #create-link-hook }
...
actually referenced by the application and provides them to link hooks via
[`LinkInput.recordedUses`][].
If `input.recordedUses` is `null`, tree-shaking is disabled and all symbols
should be kept.

1.  In your Dart project, create or open `hook/link.dart`.

1.  In the `main` method, call the `link` function from
    `package:hooks/hooks.dart` and pass tree-shaking options to [`CLibrary.link`][].
    For example:

    ```dart title="hook/link.dart"
    import 'package:hooks/hooks.dart';
    import 'package:native_toolchain_c/native_toolchain_c.dart';
...
through `recordUseMapping` to pass the corresponding native symbols to
`LinkerOptions.treeshake`.

When `symbolsToKeep` is `null` (for example, when `input.recordedUses` is
`null`), `LinkerOptions.treeshake` preserves all symbols in the native library.
When `symbolsToKeep` is an empty list (`[]`), meaning no symbols from the
library are referenced in the application, `CLibrary.link` automatically skips
...
[Record use package]: {{site.pub-pkg}}/record_use
[Record use library reference]: {{site.pub-api}}/record_use/latest/record_use/
