...
- question: >-
  ...
  options:
    - text: >-
        When you need a type that can only be one of
        a fixed, known set of values.
      correct: true
      explanation: >-
        Enums are perfect when you have a closed set of options,
        like `flag` versus `option`, days of the week, or status codes.
        The compiler ensures you handle all cases.
    - text: >-
        When you need to store user input that could be any string.
      correct: false
      explanation: >-
        User input is unpredictable and could be anything.
        Enums work best when you can list every possible value in advance.
    - text: >-
      ...
      explanation: >-
        ...
        there's only one instance for each value.
    - text: >-
        When you need to perform arithmetic operations on the values.
      correct: false
      explanation: >-
        Use `int` or `double` for arithmetic.
        Enums represent distinct categories, not numeric values
        (though they do have an index).
</Quiz>
