...
We’ve made several improvements over the last few releases, such as simplifying code for [constructing widgets]({{site.dart-blog}}/announcing-dart-2-optimized-for-client-side-development#ui-as-code), adding automatic [int-to-double conversion]({{site.dart-blog}}/announcing-dart-2-1-improved-performance-usability), and adding [set literals]({{site.dart-blog}}/announcing-dart-2-2-faster-native-code-support-for-set-literals). In Dart 2.3 we take another big step forward, with three new features for expressing UI that is list-based, conditional, or repeated.
...
For details on all the work that was done to add these features, check out [this recent post]({{site.dart-blog}}/making-dart-a-better-language-for-ui) from Bob Nystrom (aka [munificentbob](https://twitter.com/munificentbob)), an engineer on the Dart language team.
