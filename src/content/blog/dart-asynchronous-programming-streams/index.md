...
***Note**: You might want to check out a [version of this article where the code is updated to be null safe]({{site.dart-blog}}/dart-asynchronous-programming-streams). (The video has not changed.)*
...
If you’ve read the [previous article on futures]({{site.dart-blog}}/dart-asynchronous-programming-futures), you might remember that **each future represents a single value** (either an error or data) that it delivers **asynchronously**. Streams work similarly, but instead of a single thing, **a stream can deliver zero or more values and errors** over **time.**
> This article is the third one based on the *Flutter in Focus* video series *Asynchronous Programming in Dart*. The first article, [Isolates and event loops]({{site.dart-blog}}/dart-asynchronous-programming-isolates-and-event-loops), covered the foundations of Dart’s support for background work. The second one, [Futures]({{site.dart-blog}}/dart-asynchronous-programming-futures), discussed the `Future` class.
