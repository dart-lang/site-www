# Dart blog post format

Use this reference for new posts, imports, metadata or media changes,
structural edits, and full reviews in `src/content/blog/`.
...
```text
- src/content/blog/<stable-kebab-case-slug>/
  - index.md
  - images/
```
...
`category` (optional)
: A valid category slug.
  Available categories include `releases`, `announcements`, `deep-dive`, and
  `other` (defaults to `other` when omitted).
...
```markdown
<DashImage src="images/workflow.webp" alt="Diagram showing requests moving from the Dart app through the API to the cache" caption="Requests move through the API before reaching the cache." />
```
...
- Use root-relative paths for internal blog links,
  such as `/blog/<slug>`.
- Use site-relative paths for docs pages,
  such as `/tools/pub`.
